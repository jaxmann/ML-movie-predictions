...
class BoxOfficeMojo(object):
    ...
    def get_all_movies(self):
        count = -1
        movie_data = open('all_movies_summary_data.csv', 'w')
        ...
        for key, val in self.movie_urls.iteritems():
            ...
            if count == -1:
                header = movie_json.keys()
                csvwriter.writerow(header)
                count += 1

            csvwriter.writerow(movie_json.values())

            # print movie.to_json()

    def get_all_weekly(self):
        count = -1
        weekly_data = open('all_movies_weekly_data.csv', 'w')

        csvwriter = csv.writer(weekly_data)

        for key, val in self.movie_urls.iteritems():
            summary = self.get_weekly_summary(key)
            # movie.clean_data()
